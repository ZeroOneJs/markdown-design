...
::: tip 提示
如果你使用 Render 组件实例作为搜索目标元素，大部分情况下不需要手动刷新，因为 Search 组件会在内容变化时自动刷新搜索结果。
:::
...
| blur    | 使搜索框失去焦点 | `() => void`                                                            |
| clear   | 清空搜索内容     | `() => void`                                                            |
| toggle  | 切换当前索引值   | `(index: number \| 'prev' \| 'next', ignoreDisabled?: boolean) => void` |
